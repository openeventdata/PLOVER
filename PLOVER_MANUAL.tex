...
As with the events, the PLOVER specification seeks to pare this down to the most commonly used entities and agent/sectors, while retaining the possibility of more specific information. In place of the pages of entity and agent specification found in the CAMEO manual, PLOVER has five rules:

\begin{enumerate}
\item The entity code---\texttt{actor/recipient}---is either an ISO 3 letter country code for independent states, a small number of non-independent territories that have assigned ISO codes, or one of a small number of non-state codes such as IGO; see Table \ref{tab:nonstate} below.
\item The country code is the ISO 3 letter country code to which an entity belongs. For territories and other special non-independent entities it indicates the state they are part of; in all other cases it is equal to the entity code. For example, for the entity Hong Kong (HKG), this field has the ISO code for China, CHN.
\item The sector code is a 3-character primary code that best describes an actors role; any other applicable sector codes are listed in a secondary field.
...
\section{Entity and country codes}

There are three groups of entities:

\begin{enumerate}
\item Independent states according to the Correlates of War state system membership list, identified via their ISO 3166 3-letter codes.\fn{\txt{https://en.wikipedia.org/wiki/ISO\_3166-1\_alpha-3} ISO codes are also available for dependencies such as the \AA land Islands, and the list is updated fairly regularly to accommodate new states such as South Sudan.}.
\item By exception, a small number of non-independent territories and dependencies with a ISO 3166 code, like Hong Kong.
\item A small number of non-state entities, listed in Table \ref{tab:nonstate}.
\end{enumerate}

The country codes field can only take on values corresponding to states recognized by the US or one of the special codes listed below.

With the exception of the UN and EU, the special codes in in Table \ref{tab:nonstate} are intentionally broad and generic. The EU and UN were separated from IGO because they are (a) fairly common, and (b) the largest and arguable among the most institutionalized IGOs.
...
Code & Content \\
\hline
IGO   &   international governmental organization, excluding the UN and EU \\
UNO & United Nations \\
EUR & European Union \\
...
~   & \textbf{Less frequently used codes}\\
 \hline
      ...
      ELI & former government officials who currently do not hold positions in the government or armed forces\\
